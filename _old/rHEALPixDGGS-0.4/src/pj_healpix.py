r"""
This Python 2.7 module implements the HEALPix map projection as described in [CaRo2007]_.
...
def healpix_diagram(a=1, e=0, shade_polar_region=True):
    ...
    g += point([pn + k*dl for k in xrange(4)] +
               [ps + k*dl for k in xrange(4)], size=20, color=color)
    g += point([pn + k*dl for k in xrange(1, 4)] + 
               [ps + k*dl for k in xrange(1, 4)], color='white', size=10, 
               zorder=3)
