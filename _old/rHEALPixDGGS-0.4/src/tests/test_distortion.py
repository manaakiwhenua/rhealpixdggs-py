...
class MyTestCase(unittest.TestCase):
    ...
    def test_fff_coeffs(self):
        ...
        E, F, G = fff_coeffs(cea, lam, phi)
        self.assertTrue(rel_err(E, sphere.R**2) < epsilon)
        self.assertAlmostEqual(F, 0)
        self.assertTrue(rel_err(G, (sphere.R*cos(phi))**2) < epsilon) 
    
        # Degrees mode output should agree with radians mode output.
        get = fff_coeffs(cea_ed, lam_deg, phi_deg)
        expect = fff_coeffs(cea_e, lam, phi)
    ...
    def test_scale_factors(self):
        ...
        s_M, s_P, s_A, theta = scale_factors(merc, lam, phi)
        ...
        s_M, s_P, s_A, theta = scale_factors(cea ,lam, phi)
        self.assertTrue(rel_err(s_M, cos(phi)) < epsilon)
        self.assertTrue(rel_err(s_P, 1/cos(phi)) < epsilon)
        self.assertTrue(rel_err(s_A, 1) < epsilon)
        self.assertTrue(rel_err(theta, pi/2) < epsilon)

        s_M, s_P, s_A, theta = scale_factors(cea_e, lam, phi)
        a = ellipsoid.a
        e = ellipsoid.e
        R_A = ellipsoid.R_A
        ...
        # Degrees mode output should agree with radians mode output.
        get = scale_factors(cea_ed, lam_deg, phi_deg)
        expect = scale_factors(cea_e, lam, phi)
        for i in range(len(expect)):
            self.assertAlmostEqual(get[i], expect[i])
    ...
    def test_utm_zone(self):
        ...
        self.assertEqual(utm_zone(lam, phi), 1)
        self.assertEqual(utm_zone(-lam, -phi), 60)
        lam, phi = deg2rad((-15, 0))
        self.assertEqual(utm_zone(lam, phi), 28)
        self.assertEqual(utm_zone(0, 85*pi/180), None)
        self.assertEqual(utm_zone(0, -84*pi/180), None)
