r"""
This Python 2.7 code tests the ``projection_wrapper`` module.
...
#------------------------------------------------------------------------------# Test Proj class.
    def test_call(self):
        ...
        f = Proj(ellipsoid=E, proj='rhealpix', 
        ...
        # Test degrees mode
        E_DEG = WGS84_ELLIPSOID
        f_deg = Proj(ellipsoid=E_DEG, proj='rhealpix', 
                     north_square=ns, south_square=ss)
        ...
        p = (pi/3, -pi/6)
        z = (0, 0)
        E = Ellipsoid(R=5, lon_0=p[0], lat_0=p[1])
        f = Proj(ellipsoid=E, proj='rhealpix', 
                 north_square=ns, south_square=ss)
        ...
        p = (-20, 50)
        z = (0, 0)
        E_DEG = Ellipsoid(R=5, lon_0=p[0], lat_0=p[1], radians=False)
        f = Proj(ellipsoid=E_DEG, proj='rhealpix', 
                 north_square=ns, south_square=ss)
        self.assertEqual(f(*p), z)
        self.assertEqual(f(*z, inverse=True), p)
#------------------------------------------------------------------------------                           
