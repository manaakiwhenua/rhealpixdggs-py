...
# Add any Sphinx extension module names here, as strings. They can b
...
# built documents.
#
# Read the version from pyproject.toml (the single source of truth for it)
# so these docs can't silently drift out of date again. Parsed with a plain
# regular expression rather than a TOML parser so building the docs needs
# nothing beyond Sphinx itself.
import pathlib
import re

_pyproject = (pathlib.Path(__file__).parents[2] / "pyproject.toml").read_text()
release = re.search(r'^version\s*=\s*"([^"]+)"', _pyproject, re.MULTILINE).group(1)
...
man_pages = [
    ("index", "rHEALPixDGGS", "rHEALPixDGGS Documentation", ["Alexander Raichev"], 1)
]
