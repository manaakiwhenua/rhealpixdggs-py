r"""
This module computes linear and areal distortion statistics of map projections.
It is used for analysis only and so is not essential to manipulating the rHEALPix discrete global grid system.
...
# Import my modules.
from projection_tools import wrap_longitude, wrap_latitude
...
def scale_factors(T, lam, phi):
    ...
    EXAMPLES::
        ...
        >>> from ellipsoids import WGS84_ELLIPSOID_RADIANS
        >>> f = Proj(ellipsoid=WGS84_ELLIPSOID_RADIANS, proj='healpix')
        >>> print scale_factors(f, 0, pi/6)
        (1.0212575853790069, 1.1535746974071357, 1.1780969100283301, 1.5707963267948966)
    """
...
def distortion(T, lam, phi):
    ...
    EXAMPLES::
        ...
        >>> from ellipsoids import WGS84_ELLIPSOID_RADIANS
        >>> f = Proj(ellipsoid=WGS84_ELLIPSOID_RADIANS, proj='healpix')
        >>> print distortion(f, 0, pi/6)
        ((0.0, 3748655.1150495009), 0.12175548293070705, 1.1295629172526771, 1.1780969100283296)
    """
    x, y = T(lam, phi) 
...
def distortion_stats(T, sample, round_numbers=3):
    r"""
    ...
    if round_numbers:
        new_stats = []
        for x in stats:
             new_stats.append([round(xx, round_numbers) for xx in x])
        stats = new_stats
