r"""
This Python 2.7 code tests the ``PJ_healpix.c`` code of the third party
pyproj module.
...
from scipy import array, sin, arcsin, pi, sqrt, rad2deg
...
class PJHEALPixTestCase(unittest.TestCase):
    ...
    def test_rhealpix_sphere(self):
        ...
        expect = [h(*p, radians=True) for p in given]
        for (ns, ss) in product(range(4), repeat=2): 
            ...
            rh = Proj(proj='rhealpix', R=R, north_square=ns, 
                      south_square=ss)
            get = [rh(*p, radians=True) for p in given]
            for i in range(len(get)):
                print given[i], expect[i], get[i]
        ...
        south_healpix_output = [R*array((-pi + i*pi/2 + dx, -pi/4 - dy)) 
                                for i in range(4)] 
        h = Proj(proj='healpix', R=R)
        north_given = [h(*p, radians=True, inverse=True) 
                       for p in north_healpix_output]
        south_given = [h(*p, radians=True, inverse=True) 
