r"""
This Python 2.7 code tests the ``pj_rhealpix`` module.
...
from pj_rhealpix import *
from projection_tools import auth_lat
...
class MyTestCase(unittest.TestCase):
    def test_triangle(self):
        ...
        for (north_square, south_square) in product(range(4), repeat=2): 
            for p in eq_points:
                cr = triangle(*p, north_square=north_square, 
    ...
    def test_combine_triangles(self):
        ...
        for (north_square, south_square) in product(range(4), repeat=2): 
            for p in eq_points:
                q = combine_triangles(*p, north_square=north_square, 
                                 south_square=south_square)
        ...
        for (north_square, south_square) in product(range(4), repeat=2): 
            for p in eq_points + sp_points + np_points:
                q = combine_triangles(*p, south_square=south_square, 
                                 north_square=north_square)
                pp = combine_triangles(*q, south_square=south_square, 
        ...
        for (north_square, south_square) in product(range(4), repeat=2): 
            ...
            for q in np_boundary:
                p = combine_triangles(*q, south_square=south_square, 
                                 north_square=north_square, inverse=True)
    ...
    def test_rhealpix_sphere(self):
        ...
        for (ns, ss) in product(range(4), repeat=2): 
            for p in inputs:
                q = rhealpix_sphere(*p, north_square=ns, south_square=ss)
                qq = combine_triangles(*healpix_sphere(*p), north_square=ns, 
                                  south_square=ss)
                qq = tuple(qq)
        ...
        for (ns, ss) in product(range(4), repeat=2): 
            for p in inputs:
                q = rhealpix_sphere(*p, north_square=ns, south_square=ss)
                pp = rhealpix_sphere_inverse(*q, north_square=ns, 
                                             south_square=ss)                
                self.assertTrue(euclidean(p, pp) < error)
    ...
    def test_rhealpix_ellipsoid(self):
        ...
        for (ns, ss) in product(range(4), repeat=2): 
            for p in inputs:
                q = rhealpix_ellipsoid(*p, north_square=ns, south_square=ss, 
                                       e=e)
        ...
        for (ns, ss) in product(range(4), repeat=2): 
            for p in inputs:
                q = rhealpix_ellipsoid(*p, north_square=ns, south_square=ss, 
                                       e=e)
                pp = rhealpix_ellipsoid_inverse(*q, north_square=ns, 
                                                south_square=ss, e=e)                
