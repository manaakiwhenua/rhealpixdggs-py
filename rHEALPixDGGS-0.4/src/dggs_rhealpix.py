...
class RHEALPixDGGS(object):
    ...
    def __init__(self, ellipsoid=WGS84_ELLIPSOID, N_side=3,        
        ...
        #
        child_order = {}   
        for (row, col) in product(range(N_side), repeat=2):    
    ...
    def rhealpix(self, u, v, inverse=False):
        ...
        EXAMPLES::
            ...
            >>> rdggs = UNIT_003_RADIANS
            >>> print rdggs.rhealpix(0, pi/3)
            (-1.8582720066840039, 2.0687188103032375)
    ...
    def combine_triangles(self, u, v, inverse=False):
        ...
        EXAMPLES::
            ...
            (0, 0)
            >>> print rdggs.combine_triangles(*q)
            (-2.3561944901923448, 1.5707963267948966)
        """
    ...
    def xyz(self, u, v, lonlat=False):
        ...
        EXAMPLES::
            ...
            >>> rdggs = UNIT_003_RADIANS
            >>> print rdggs.xyz(0, pi/4, lonlat=True)
    ...
    def xyz_cube(self, u, v, lonlat=False):
        ...
        EXAMPLES::
            ...
            >>> rdggs = UNIT_003
            >>> print rdggs.xyz_cube(0, 0)
            (0.78539816339744828, 0.0, -0.78539816339744828)
        """
        if lonlat:
    ...
    def num_cells(self, res_1, res_2=None, subcells=False):
        ...
        if subcells:
            ...
            num = (k**(res_2 - res_1 + 1) - 1)/(k - 1)
        else:
            if (res_2 is None) or (res_2 < res_1):
                res_2 = res_1
            num = 6*(k**(res_2 + 1) - k**res_1)/(k - 1)
        return num
    ...
    def minimal_cover(self, resolution, points, plane=True):
        ...
        EXAMPLES::
            ...
            >>> for r in range(5):
            ...     cover = rdggs.minimal_cover(r, points)
...
class Cell(object):
    ...
    def __init__(self, rdggs=WGS84_003, suid=None, 
        ...
        EXAMPLES::
            ...
            >>> print c
            N3
            >>> c = Cell(rdggs, level_order_index=6 + 6*9 - 1)
            >>> print c
            S8
    ...
    def successor(self, resolution=None):
        ...
        M = self.N_side**2 - 1
        greatest = 0
        for i in reversed(range(1, resolution + 1)):
            if suid[i] != M:
                greatest = i
                break
    ...
    def subcells(self, resolution=None):
        ...
        if resolution == L:
            ...
            return 
        N = self.N_side
        for t in product(range(N**2), repeat=resolution - L):
    ...
    def nw_vertex(self, plane=True):
        ...
        EXAMPLES::
            ...
            >>> print c.ul_vertex(plane=True)
            (-2225148.7007489, -556287.17518722452)
            >>> print c.nw_vertex(plane=True)
            (-2225148.7007489, -556287.17518722452)
    ...
    def vertices(self, plane=True, trim_dart=False):
        ...
        EXAMPLES::
            ...
            (-3.1415926535897931, 0.78539816339744828)
    ...
    def boundary(self, n=2, plane=True, interior=False):
        ...
        EXAMPLES::
            ...
            ...     print p
            (-3.1415926535897931, 1.3089969389957472)
            (-2.8797932657906435, 1.3089969389957472)
            (-2.6179938779914944, 1.3089969389957472)
            (-2.6179938779914944, 1.0471975511965979)
            (-2.6179938779914944, 0.78539816339744839)
            (-2.879793265790644, 0.78539816339744839)
            (-3.1415926535897931, 0.78539816339744839)
            (-3.1415926535897931, 1.0471975511965979)
            ...
            (165.0, 41.810314895778596)
            (149.99999999999997, 41.810314895778596)
    ...
    def interior(self, n=2, plane=True, flatten=False):
        ...
        EXAMPLES::
            ...
            (-1.2722218725854067e-14, 41.810380145353903)
            (-90.0, 41.810380145353903)
...
# Probably don't need cell family objects any more for SCENZ-Grid, 
# because these will probably be handled by a graph database.
...
#     r"""
#     A subcell-free ordered list of cells along with the minimum resolution of 
#     the cells in family the maximum resolution of the cells in family.
#     """
#      
#     def __init__(self, cells=None, min_resolution=None, max_resolution=None,
...
#         len(cells).
#         If `sort=False` and (`eliminate_subcells=True` or `min_resolution=None` or 
#         `max_resolution=None`), then uses O(n) cell operations.
...
#             self.rdggs = None
#             # The resolution of the biggest cells in the family:        
#             self.min_resolution = None
#             # The resolution of the smallest cells in the family:
#             self.max_resolution = None
#         else:
#             self.cells = cells
#             self.rdggs = cells[0].rdggs  
...
#                 self.min_resolution = self.cells[0].resolution
#                 self.max_resolution = self.min_resolution
#                 for c in self.cells:
#                     self.min_resolution = min(self.min_resolution, c.resolution)
#                     self.max_resolution = max(self.max_resolution, c.resolution)
#                 
...
#         
#     def union(self, other, filter_resolution=None):
#         r"""
...
#         If a nonnegative integer `filter_resolution` is given, then return the cell
#         family comprised of the cell's of u at resolutions <= `filter_resolution`.
...
#         EXAMPLES::
#         
#             >>> a = Cell(RHEALPixDGGS(), 'N')
#             >>> b1 = Cell(RHEALPixDGGS(), 'P1')
...
#             >>> print f.union(g)
#             ['N', 'P1']
#             >>> print f.union(g, filter_resolution=0)
#             ['N']
# 
#         """
#         f = self
...
#         # We can't ignore cells of resolution > filter_resolution,
#         # because they may aggregate to bigger (smaller resolution) cells.    
#         u = []  # The union  
...
#  
#         # Filter by resolution if desired.
#         if isinstance(filter_resolution, int) and filter_resolution >= 0 \
#         and filter_resolution < u.max_resolution:
#             u = CellFamily([x for x in u if x.resolution <= filter_resolution], 
#                            sort=False, eliminate_subcells=False)
#         return u
#     
#     def union_all(self, others, filter_resolution=None, minimize_first=True):
#         r"""
#         Return the cell family u that is the union of the regions of `self` 
#         and cell families in the list `others`. 
#         If a nonnegative integer `filter_resolution` is given, then return the cell
#         family comprised of the cell's of u at resolutions <= `filter_resolution`.
# 
...
#         return u
#         
#     def intersect(self, other, filter_resolution=None):
#         r"""
#         Return the minimal cell family v for the intersection of the regions 
#         of `self` and `other`. 
#         If a nonnegative integer `filter_resolution` is given, then return the cell
#         family comprised of the cell's of v at resolutions <= `filter_resolution`. 
#  
#         Uses O(max(m, n)) cell operations, where m = `len(self)` and
#         n = `len(other)`.
#         
...
#             >>> print f.intersect(g)
#             ['N3', 'P']
#             >>> print f.intersect(g, filter_resolution=0)
#             ['P']
#             
#         """
#         f = self
...
#             return CellFamily()       
#             
#         # Set default filter resolution if none given.
#         if not (isinstance(filter_resolution, int) and filter_resolution >= 0):
#             filter_resolution = f.rdggs.max_resolution
#              
#         # Intersect the regions of f and g,
#         # ignoring cells at resolution > filter_resolution.
#         v = []  # The intersection.
#         i = 0   # Index of current cell in f under investigation.
#         j = 0   # Index of current cell in g under investigation.
#         while i < len(f) and j < len(g):
#             a = f[i]
#             b = g[j]
#             if a.resolution > filter_resolution:
#                 # Discard/skip a.
#                 i += 1
#                 continue
#             if b.resolution > filter_resolution:
#                 # Discard/skip b.
#                 j += 1
#                 continue
#             if a.subcell(b):
...
#         
#     def intersect_all(self, others, filter_resolution=None):
#         r"""
#         Return the minimal cell family v for the intersection of the regions
...
#         v = families.pop(0)
#         for f in families:
#             v = v.intersect(f, filter_resolution=filter_resolution)
#             if not v:
