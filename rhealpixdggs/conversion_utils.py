...
def call_get_finest(feature):
    ...
    for suid in [tuple(x) for x in ['N', 'O', 'P', 'Q', 'R', 'S']]:
        finest = _get_finest_cell(feature, suid)
        if finest is not None:
            return finest


def _get_finest_cell(feature, suid):
    ...
    # function and truth list for multipolygon / polygon (feature) contained within multipolygon / polygon (cell)
    truth = [poly.contains(feature) for poly in children_poly]
    # if we get something back, check the next level lower
    returned_cells = list(compress(children_cells, truth))
    if returned_cells:
        finest = _get_finest_cell(feature, returned_cells[0].suid)
    else:
        parent_poly = Polygon(parent_cell.vertices(plane=False))
        if parent_poly.contains(feature):
            finest = parent_cell
        else:
            finest = None
    return finest
...
class PolygonAsDGGS:
    ...
    def get_dggs_poly(self, bounding_cell):
        bounding_poly = Polygon(bounding_cell.vertices(plane=False))
        if self.feature.contains(bounding_poly):  # edge case where the feature is the same as the bounding cell
            self.write_cells(bounding_cell, bounding_poly, 'bounding poly')
        else:
            if bounding_cell.resolution + 1 > self.res_limit:
                ...
                self.process_children(together)
    ...
    def process_children(self, together):
        for child_cell, child_poly in together:
            # 1: add contained cells
            if self.feature.contains(child_poly):
                self.write_cells(child_cell, child_poly, 'fully contained')
            # 2: check we're not at the limit, if we are, check centroids
            elif child_cell.resolution == self.res_limit:
                if self.feature.contains(Point(child_cell.nucleus(plane=False))):
                    self.write_cells(child_cell, child_poly, 'nucleus')
            # 3: check the children (call this same function on the children)
            else:
                if self.feature.overlaps(child_poly):
                    self.get_dggs_poly(child_cell)


    def write_cells(self, cell, poly, desc):
        """
...
with open(file_out_path, 'a') as file:
    for row in act_geoms.itertuples():
        ...
        cells_list = PolygonAsDGGS(feature=(row.Geom_ID, row.Geometries),
